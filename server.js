...
app.get('/info', async (req, res) => {
  try {
      ...
      if (format.container && format.qualityLabel) {
        ...
        if (!seen.has(key)) {
          ...
          uniqueFormats.push({
            ...
            fileSize: (format.contentLength / (1024 * 1024)).toFixed(2) + ' MB',
    ...
    uniqueFormats.push({
      ...
      fileSize: (videoDuration * 320 * 1024 / 8 / (1024 * 1024)).toFixed(2) + ' MB',
      container: 'mp3',
      audioOnly: true
    });
    ...
    uniqueFormats.push({
      ...
      fileSize: (videoDuration * 128 * 1024 / 8 / (1024 * 1024)).toFixed(2) + ' MB',
      container: 'mp3',
      audioOnly: true
    });
...
app.get('/download', async (req, res) => {
  ...
  if (itag.startsWith('mp3')) {
    ...
    videoStream.on('progress', (chunkLength, downloaded, total) => {
      ...
      if (debugMode) {
        logStream.write(`Download progress: ${percent.toFixed(2)}%\n`);
      }
    });
    ...
    if (enableConversion) {
      ...
      ffmpeg()
        ...
        .outputOptions([
          ...
          '-movflags +faststart',
          '-threads 2'
        ]) // Set output options for faster encoding
        .output(outputTempPath)
        .on('start', (cmd) => {
          if (debugMode) {
            logStream.write(`FFmpeg start command: ${cmd}\n`);
